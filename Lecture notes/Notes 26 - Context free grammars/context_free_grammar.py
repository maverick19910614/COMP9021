...
# So that we can use ε in the rules we pass
# as an argument to ContextFreeGrammar
ε = ''


class ContextFreeGrammar:
    '''
    The rules of the context free grammar are a dictionnary
    ...
    >>> CFG.without_ε.rules == {'S': {'A', 'bbb', 'bSbb'}, 'A': {'aA', 'a'}}
    True
    '''
    def __init__(self, rules, starting_symbol):
        ...
        # All productions of ε are removed, possibly adding
        ...
        rules_without_ε = {nonterminal: rules[nonterminal] for nonterminal in rules
                                                  if nonterminal not in generating_ε
                          }
        for nonterminal in generating_ε:
            # In a production different to ε, an occurrence of a nonterminal P
            # is either kept or eliminated (doubling the number of
            # derived productions) in case ε can be generated from P.
            # For instance, if X can generate ε but Y cannot, then
        ...
        self.without_ε = ContextFreeGrammarWithoutε(rules_without_ε, self.starting_symbol)
    ...
    def __str__(self):
        return '\n'.join(' -> '.join((f'{nonterminal}',
                                    ...
                                    ) for nonterminal in self._ordered_nonterminals()
                        )
    ...
    def can_generate_with_no_ε(self, word):
        ...
        >>> CFG = ContextFreeGrammar({'S': {'A', 'bbb', 'bSbb'}, 'A': {'aA', 'a'}}, 'S')
        >>> CFG.can_generate_with_no_ε('')
        False
        ...
        while generated_bigrams:
            ...
            for pattern in self.rules[w_2[0]]:
                ...
                if len(new_bigram[0]) + len(new_bigram[1]) <= len(word) and\
                                                            new_bigram not in seen_bigrams:
