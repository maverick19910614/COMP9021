...
class FiniteProbabilityDistribution:
    ...
    def __init__(self, μ):
        if isinstance(μ, Set):
            if not len(μ):
                raise FiniteProbabilityDistributionError('The sample space should not be empty')                
    ...
    def conditionalised_by(self, event):
        ...
        return FiniteProbabilityDistribution({outcome: self.μ[outcome] /
    ...
    def __str__(self):
        return '\n'.join(outcome + ' : ' + str(self.μ[outcome]) for outcome in sorted(self.μ))    
...
class Event:
    def __init__(self, distribution, outcomes = set()):
        ...
        self.probability = sum(self.distribution.μ[outcome] for outcome in self.outcomes)
    
    def __invert__(self):
        return Event(self.distribution, set(self.distribution.μ) - self.outcomes)
    ...
    def __or__(self, other):
        ...
        return Event(self.distribution.conditionalised_by(other), self.outcomes & other.outcomes)
