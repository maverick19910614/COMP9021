...
class NashEquilibriumCalculator(tk.Tk):
    ...
    def analyse_game(self, payoffs_per_player):
        # Let a1, a2, a3 and a4 denote payoffs[0][0][0], payoffs[0][0][1],
        # payoffs[0][1][0] and payoffs[0][1][1], respectively.
        # Let b1, b2, b3 and b4 denote payoffs[1][0][0], payoffs[1][0][1],
        # payoffs[1][1][0] and payoffs[1][1][1], respectively.
        ...
        # self.expectations['Ruth'].
        # When there exists a unique p in [0, 1] with D[1] * p + E[1] == 0,
        # then that p is recorded as probas['Charlie'];
        ...
        for player in players:
            ...
            D[player] = payoffs[0][0] - payoffs[0][1] - payoffs[1][0] + payoffs[1][1]
            E[player] = payoffs[int(player == 'Ruth')][int(player == 'Charlie')] - payoffs[0][0]
            if D[player]:
                ...
                if cut < 0 or cut > 1:
                    ...
                    if cut in {0, 1}:
                        self.segments[player][int((cut == 1) ^ (D[player] > 0))] = 0, 1
                    else:
                        F[player] = payoffs[int(player == 'Charlie')][int(player == 'Ruth')] -\
                                                                                      payoffs[0][0]
                        self.expectations[player] = F[player] * self.probas[player] + payoffs[0][0]
                        self.segments[player][int(D[player] < 0)] = 0, self.probas[player]
                        self.segments[player][int(D[player] > 0)] = self.probas[player], 1
            elif E[player]:
                self.segments[player][1 - int(E[player] < 0)] = 0, 1
            else:
    ...
    def is_pure_equilibrium(self, i, j):
        return all(X[0] in self.all_good or\
                   self.segments[X[0]][X[1]] and self.segments[X[0]][X[1]][X[2]] == X[2] or\
                   self.probas[X[0]] == j for X in {(players[0], i, j), (players[1], j, i)}
                  )

    def draw_rectangle(self, colour):
        self.graph.create_rectangle(5, 5, 105, 105, fill = colour, outline = colour)
        ...
        for i in range(2):
            for player_1, player_2 in {players, reversed(players)}:
                if self.segments[player_1][i]:
                    ...
                    # an inner line segment S' (of length 1) for the other player,
                    # then S' has been drawn already.
                    # The intersection of S and S' is drawn again using 'Nash' colour.
                    colour = self.colours[player_1] if player_2 not in self.all_good and\
                                                      (self.probas[player_2] is None or\
                                                       self.probas[player_2] != i)\
                                                    else self.colours['Nash']
                    self.draw_line(*x_y[player_1](i, self.segments[player_1][i][0],
                                                     self.segments[player_1][i][1]
                                                 ), colour
                                  )

    def draw_inner_lines(self):
        x_y = {'Ruth': lambda p: (5, 100 * (1 - p) + 5, 105, 100 * (1 - p) + 5),
               'Charlie': lambda p: (100 * p + 5, 5, 100 * p + 5, 105)
              }
        for player_1, player_2 in {players, reversed(players)}:
            if self.probas[player_1] is not None:
                if player_2 in self.all_good:
                    self.draw_line(*x_y[player_1](self.probas[player_1]), self.colours['Nash'])
                else:
                    self.draw_line(*x_y[player_1](self.probas[player_1]), self.colours[player_1])

    def draw_intersecting_nash_equilibria(self):
        if self.expectations['Ruth'] is not None and self.expectations['Charlie'] is not None:
            self.graph.create_oval(self.probas['Charlie'] * 100 + 3,
                                   ...
                                   fill = self.colours['Nash'], outline = self.colours['Nash']
                                  )
    ...
    def display_special_information(self):
        if self.expectations['Ruth'] is not None:
            self.special_expectations[0].set(f"When Charlie uses proba q = {self.probas['Ruth']:.2f}"
                                        f", Ruth's expectation is {self.expectations['Ruth']:.2f}"
                                            )
        if self.expectations['Charlie'] is not None:
            self.special_expectations[1].set(f"When Ruth uses proba p = {self.probas['Charlie']:.2f}"
...
class GameMatrix(tk.Frame):
    ...
    def create_matrix(self):
        # The payoffs will be analysed as if they were entered
        # from the pair that Ruth and Charlie would chose
        ...
        self.payoffs_per_player = {player: [[None] * 2 for _ in range(2)] for player in players}
        for i, j in product(range(2), repeat = 2):
        ...
        tk.Label(self, text = 'Ruth', fg = NashEquilibriumCalculator.colours['Ruth']).grid(
                                                                           columnspan = 2, pady = 5
                                                                                      )
        tk.Label(self, text = 'Charlie', fg = NashEquilibriumCalculator.colours['Charlie']).grid(
    ...
    def get_payoffs(self):
        ...
        for i, j in product(range(2), repeat = 2):
            ...
            if not parsed_input:
                ...
                self.cells[i][j].config(bg = NashEquilibriumCalculator.cell_colour)
                self.payoffs_per_player['Ruth'][i][j] = int(parsed_input.groups()[0])
                self.payoffs_per_player['Charlie'][i][j] = int(parsed_input.groups()[1])
