...
Illustrates the use of a deque for depth-first and breadth-first search for a solution.
...
class Conjunction(list):
    ...
    def predicate_and_function_symbols(self):
        ...
        function_symbols = {}
        for atom in self:
            atom_predicate_symbol, atom_function_symbols = atom.predicate_and_function_symbols()
            if not consistently_add_to(atom_predicate_symbol, predicate_symbols):
                raise Conjunction.ConjunctionError(f'{self}: predicate symbol used with many arities')
            if not consistently_merge_to(atom_function_symbols, function_symbols):
                raise Conjunction.ConjunctionError(f'{self}: function symbol used with many arities')
        if predicate_symbols.keys() & function_symbols.keys():
            raise Conjunction.ConjunctionError(f'{self}: symbol used as predicate and function symbol')
...
class Rule:
    ...
    def predicate_and_function_symbols(self):
        ...
        if not self.body:
            return {head_predicate_symbol[0]: head_predicate_symbol[1]}, head_function_symbols
        ...
        if not consistently_merge_to(head_function_symbols, function_symbols):
            raise Rule.RuleError(f'{self}: Function symbol used with different arities '
                                 'in head and body'
                                )
        if predicate_symbols.keys() & function_symbols.keys():
            raise Rule.RuleError(f'{self}: symbol used as predicate and function symbols, '
                                 'one in head, the other in body'
                                    )
        return predicate_symbols, function_symbols
    ...
    def parse_rule(expression):
        ...
        >>> rule = Rule.parse_rule('grandparent(X, Y) :- parent(X, Z), parent(Z, Y).')
        >>> print(rule.head)
        grandparent(X, Y)
        >>> for atom in rule.body: print(atom)
        parent(X, Z)
        ...
        if len(rule) == 1:
            ...
            if not body:
                raise Rule.RuleError(f'{expression}: syntactically incorrect body')
        ...
        with open(filename) as file:
            for rule in file:
                ...
                rule_predicate_symbols, rule_function_symbols =\
                                    rule.predicate_and_function_symbols()
                if not consistently_merge_to(rule_predicate_symbols, self.predicate_symbols) or\
                   not consistently_merge_to(rule_function_symbols, self.function_symbols):
                    raise LogicProgram.LogicProgramError(f'Symbol arity in rule nb {rule_nb} '
                                                         'inconsistent with arities in previous rules'
                                                        )
                rule_variables = rule.head.variables()
                for atom in rule.body:
                    rule_variables |= atom.variables()
                underscore_index = rule.head.individualise_underscores(rule_variables)
                for atom in rule.body:
                    underscore_index = atom.individualise_underscores(rule_variables, underscore_index)
                self.program.append(rule)
        if self.predicate_symbols.keys() & self.function_symbols.keys():
            raise LogicProgram.LogicProgramError('symbol used as predicate and function symbols')

    def solve(self, query, depth_first = True):
        query = Conjunction(Expression.parse_item(query, parse_single_subitem = False))
        query_predicate_symbols, query_function_symbols = query.predicate_and_function_symbols()
        if any(predicate_symbol not in self.predicate_symbols or
               query_predicate_symbols[predicate_symbol] != self.predicate_symbols[predicate_symbol]
                   for predicate_symbol in query_predicate_symbols
              ):
            raise LogicProgram.QuerryError(f'{query}: predicate symbol in query not in program')
        if any(function_symbol not in self.function_symbols or
               query_function_symbols[function_symbol] != self.function_symbols[function_symbol]
                   for function_symbol in query_function_symbols
              ):
            raise LogicProgram.QuerryError(f'{query}: function symbol in query not in program')               
        ...
        goals_solution_pairs = deque([(deque(query), {var: Term(var) for var in query_variables})])
        while goals_solution_pairs:
            goals, solution = goals_solution_pairs.popleft()
            if not goals:
                yield {var: solution[var].__str__() for var in solution}
                continue
            reserved_variables = query_variables | {var for atom in goals for var in atom.variables()}
            goal = goals.popleft()
            next_goals_solution_pairs = deque()
            for rule in self.program:
                variable_renaming = Expression.fresh_variables(rule.variables, reserved_variables)
                head = rule.head.rename_variables(variable_renaming)
                mgu = goal.unify(head)
                if mgu is not None:
                    new_goals = deque(atom.rename_variables(variable_renaming).substitute(mgu)
                                                                       for atom in rule.body
                                     )
                    new_goals.extend(goal.substitute_in_copy(mgu) for goal in goals)
                    next_goals_solution_pairs.append((new_goals,
...
if __name__ == '__main__':
    ...
    print('A query for prolog_ex_6.pl')
    print('  Solutions to relation(f(f(f(g(f(a))))), f(f(g(g(a)))), X):')
    for solution in LP.solve('relation(f(f(f(g(f(a))))), f(f(g(g(a)))), X)', False):
        print('   ', solution)
    print()
