...
class Expression:
    ...
    def __str__(self):
        return self.root if not self.children else\
               ''.join((self.root,
                        '(',
        ...
        if self.is_variable():
            if self.root in substitution:
                self.root, self.children =\
                   substitution[self.root].root, substitution[self.root].children
    ...
    def unify(self, expression):
        ...
        >>> unifier = {X: expr.__str__() for (X, expr) in term_1.unify(term_2).items()}
        >>> unifier == {}
        True
        >>> term_1 = Term.parse_term('X')
        ...
        >>> unifier = {X: expr.__str__() for (X, expr) in term_1.unify(term_2).items()}
        >>> unifier == {'X4': 'h(X1)', 'X2': 'X3', 'X1': 'g(X3)'} or\
            unifier == {'X4': 'h(g(X3))', 'X2': 'X3', 'X1': 'g(X3)'}
        True
    ...
    def parse_word(characters):
        word = [characters.pop()]
        while characters and (characters[-1].isalnum() or characters[-1] == '_'):
    ...
    def parse_item(expression, item_type = 'Atom', parse_single_subitem = True):
        ...
        item = Expression.parse_subitem(characters, item_type) if parse_single_subitem\
    ...
    def fresh_variables(variables, reserved_variables):
        ...
        {'Y': 'Y_0'}
        >>> Expression.fresh_variables({'U', 'Y', 'Z'}, {'X', 'Y', 'Z'}) == {'Z': 'Z_0', 'Y': 'Y_0'}
        True
        '''
        substitutions = {}
        ...
        while identities:
            ...
            if expression_1.root == expression_2.root:
                identities.extend([expr_1, expr_2] for (expr_1, expr_2) in
                                             zip(expression_1.children, expression_2.children)
                                 )
                ...
                for identity in identities:
                    for i in range(2):
                        identity[i] = identity[i].substitute_in_copy({expression_1.root: expression_2})
...
father(bob, father(sandra, john, mary))
        ...
        if term.collected_symbols(True) is None:
            raise Term.TermError(f'Same function symbol used with different arities in {expression}')
        return term
...
class Atom(Expression):
    ...
    def predicate_and_function_symbols(self):
        ...
        (('bob', 0), {})
        >>> Atom.parse_atom('father(bob, sandra)').predicate_and_function_symbols()
        (('father', 2), {'bob': 0, 'sandra': 0})
        >>> Atom.parse_atom('join(l(H, T), X, l(H, Y))').predicate_and_function_symbols()
        (('join', 3), {'l': 2})
        '''
    ...
    def parse_atom(expression):
        ...
        if function_symbols is None:
            raise Atom.AtomError(f'Same function symbol used with different arities in {expression}')
