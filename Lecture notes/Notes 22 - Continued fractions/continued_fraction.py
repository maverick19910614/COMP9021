...
class ContinuedFraction:
        ...
        if periodic_expansion:
            for i in range(1, len(periodic_expansion) // 2 + 1):
                if len(periodic_expansion) % i == 0 and periodic_expansion ==\
                    ...
                    break
            while len(self.finite_expansion) > 1 and\
                                           self.finite_expansion[-1] == periodic_expansion[-1]:
            ...
            if len(self.finite_expansion) > 1 and self.finite_expansion[-1] == 1:
                self.finite_expansion.pop()
    ...
    def approximate_as_decimals(self, precision = 1):
        ...
        if self.is_rational():
            ...
            s1 = next(fraction.precision_many_decimals(abs(fraction.numerator) %
                                  fraction.denominator * 10, fraction.denominator, current_precision
                                                      )
                     )
            while True:
                fraction = next(fractions)
                s2 = next(fraction.precision_many_decimals(abs(fraction.numerator) % 
                                  fraction.denominator * 10, fraction.denominator, current_precision
                                                          )
                         )
                if s1 == s2:
    ...
    def __repr__(self):
        return f'ContinuedFraction({self.finite_expansion}, {self.periodic_expansion})'
...
class Fraction(Fraction):
    ...
    def approximate_as_decimals(self, precision = 1):
        ...
        if self.numerator > 0:
            ...
            representation = '-' + str(abs(self.numerator) // self.denominator) + '.'
        for decimals in self.precision_many_decimals(abs(self.numerator) % self.denominator * 10,
                                                     self.denominator,
                                                     precision
                                                    ):
