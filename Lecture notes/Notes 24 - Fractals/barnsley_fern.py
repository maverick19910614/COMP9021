...
class BarnsleyFern(tk.Tk):
    def __init__(self):
        ...
        for species in 'Black Spleenwort', 'Culcita', 'Cyclosorus':
            predefined_ferns.add_command(label = species,
                            command = lambda fern_species = species: self.display_fern(fern_species)
                                        )
        ...
        tk.Button(draw_buttons, text = 'Draw custom fern',
                  command = lambda fern_species = 'Custom': self.display_fern(fern_species)
                 ).pack(padx = 20, side = tk.LEFT)
        draw_buttons.pack()
        self.displayed_fern = tk.StringVar()
        tk.Label(draw_buttons, textvariable = self.displayed_fern, width = 16, fg = Fern.colour
                ).pack(pady = 40)
    ...
    def affine_transformations_help(self):
        tkinter.messagebox.showinfo('Affine transformations',
            ...
            'each of wich maps a point (x, y) to\n'
            '    (ax + by + e, cx + dy + f)\n'
            'for some real numbers a, b, c, d, e and f, i.e., A is the union of '
            'T1(A), T2(A), T3(A) and T4(A).\n\n'
            ...
            'of L and R and the tips of the largest left and right leaflets of the fern to the '
            'tips of the largest left and right leaflets of L and R, respectively.\n'
    ...
    def displayed_parameters_help(self):
        tkinter.messagebox.showinfo('Displayed parameters',
            ...
            'Factor can be used to scale the picture of the fern up or down.'
                                    )
    ...
    def fern_species_help(self):
        tkinter.messagebox.showinfo('Fern species',
            ...
            'being automatically displayed. The parameters can be set to any value to display '
            'a "custom" fern, provided that:\n'
            ' - a, b, c, d, e and f are floating point numbers;\n'
            ' - the weights are nonnegative integers that sum up to 100;\n'
...
class Drawing(tk.Frame):
    ...
    def display_fern(self, fern_species):
        if fern_species == 'Custom':
            ...
            for n in range(4):
                try:
                    self.fern.weights[n] = int(self.fern_parameters_board.weights[n].get())
                except ValueError:
                    self.fern_parameters_board.weights[n].config(bg = self.error_colour)
                    incorrect_input = True
                ...
                for i in range(2):
                    try:
                        self.fern.translation_vectors[n][i] = float(
                                          self.fern_parameters_board.translation_vectors[n][i].get()
                                                                   )
                    except ValueError:
                        self.fern_parameters_board.translation_vectors[n][i].config(
                                                                              bg = self.error_colour
                                                                                   )
                        incorrect_input = True
                    for j in range(2):
                        try:
                            self.fern.linear_coefficients[n][i][j] = float(
                                       self.fern_parameters_board.linear_coefficients[n][i][j].get()
                                                                          )
                        except ValueError:
                            self.fern_parameters_board.linear_coefficients[n][i][j].config(
                                                                              bg = self.error_colour
                                                                                          )
                            incorrect_input = True
            if incorrect_input:
            ...
            if sum(self.fern.weights) != 100:
                for n in range(4):
                    self.fern_parameters_board.weights[n].config(bg = self.error_colour)
                tkinter.messagebox.showerror('Incorrect input', 'Weights should sum to 100')
                return
        else:
            self.fern = {'Black Spleenwort': BlackSpleenwort(), 'Culcita': Culcita(),
    ...
    def draw_fern(self):
        translation_vectors = [np.array([[translation_vector[0]], [translation_vector[1]]]
                                       ) for translation_vector in self.fern.translation_vectors
                              ]
        linear_coefficients = [np.array(linear_coefficients)
                                         for linear_coefficients in self.fern.linear_coefficients
                              ]
        ...
        for _ in range(self.nb_of_iterations):
            ...
            for n in range(4):
                if r < accumulated_weights[n]:
                    point = linear_coefficients[n] @ point + translation_vectors[n]
                    x, y = point[0, 0] * self.fern.factor, point[1, 0] * self.fern.factor
                    self.drawing.create_oval(self.half_dim + x - 0.3, self.dim - y - 0.3,
                                             self.half_dim + x + 0.3, self.dim - y + 0.3,
                                             fill = Fern.colour, outline = Fern.colour
                                            )
                    break
...
class Fern:
    ...
    def __init__(self,
                 linear_coefficients = [[[None] * 2 for _ in range(2)] for _ in range(4)],
...
class BlackSpleenwort(Fern):
    def __init__(self):
        super().__init__((((0, 0), (0, 0.16)), ((0.85, 0.04), (-0.04, 0.85)),
                         ...
                         ), ((0, 0), (0, 1.6), (0, 1.6), (0, 0.44)), (1, 85, 7, 7),
                         factor = Drawing.dim // 12
                        )
...
class Culcita(Fern):
    def __init__(self):
        super().__init__((((0, 0), (0, 0.25)), ((0.85, 0.02), (-0.02, 0.83)),
                         ...
                         ), ((0, -0.14), (0, 1), (0, 0.6), (0, 0.7)), (2, 84, 7, 7),
                         factor = Drawing.dim // 6
                        )
...
class Cyclosorus(Fern):
    def __init__(self):
        super().__init__((((0, 0), (0, 0.25)), ((0.95, 0.005), (-0.005, 0.93)),
                         ...
                         ), ((0, -0.4), (-0.002, 0.5), (-0.09, 0.02), (0.083, 0.12)), (2, 94, 2, 2),
                         factor = Drawing.dim // 8
                        )
...
class FernParametersBoard(tk.Frame):
    def __init__(self):
        ...
        self.linear_coefficients = [[[None] * 2 for _ in range(2)] for _ in range(4)]
        ...
        for n in range(4):
            tk.Label(self, text = text_labels[n]).grid(row = 3 * n, columnspan = 4, pady = 5)
            for i in range(2):
                for j in range(2):
                    self.linear_coefficients[n][i][j] = tk.Entry(self, bd = 1, width = 5)
