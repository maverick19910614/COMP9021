...
# We eliminate from printable \r, \x0b (vertical tab) and \x0c (form feed)
...
def encrypt_or_decrypt(key, text, mode):
    return ''.join(characters[(shifts[text[i]] + shifts[key[i % len(key)]] * mode) % len(shifts)]
    ...
    for key_length in key_lengths_from_most_to_least_promising(text):
        ...
        for n in range(key_length):
            ...
            for subkey in characters:
                ...
                # When two subkeys have the same etaoin score, we prefer the one
                # with the highest proportion of lowercase letters over all letters
...
def all_collected_factors(text):
    # Kasiski examination
    # Collecting all factors between n and max_key_length of the distances between
    # the starts of two consecutive occurrences of an n-gram, with n in gram_range
    # (so between 3 and 5 by default). 
    # The length of the key to discover is likely to be one of the most frequent factors.
...
def key_lengths_from_most_to_least_promising(text):
    ...
    key_lengths = [x[0] for x in sorted(Counter(all_collected_factors(text)).items(),
                                        key = itemgetter(1, 0),
                                        reverse = True
                                       )
                  ]
    yield from key_lengths
    # Keys of any length at most equal to max_key_length will be tried if needed.
    yield from (i for i in range(2, max_key_length + 1) if i not in key_lengths)
