...
prompted to enter a number n of levels of a tower and
a number m of marbles.
Using those m marbles, one has to discover the highest level,
if any, such that dropping a marble from that level makes
it break, using a strategy that minimises the number of
drops in the worst case (it is assumed that any marble would
break when dropped from a level where one marble breaks, and
also when dropped from any higher level; the marbles might
not break when dropped from any level).

The idea is to ask: what is the maximum height of a tower
such that an answer can always be found with no more than d
drops? Let H(d, m) denote that maximum height.
- If the marble breaks, m - 1 marbles remain and there is no need to test higher levels.
- If the marble does not break, m marbles remain and there is no need to test lower levels.
...
For the simulation, if low is the highest level from which
...
while True:
    ...
    n = input('Enter the number of levels (a strictly positive number): ')
    try:
        n = int(n)
        if n <= 0:
            raise ValueError
...
# The highest level such that it is known that a marble dropped
# from that level does not break.
low = 0
# The smallest level such that it is known that a marble dropped
# from that level breaks (it is convenient to assume that a glass
# dropped from a level one more than the height of the tower breaks).
high = n + 1
drop = 0
marble = 1
...
while low < high - 1:
    ...
    if breaking_level <= level:
       print(f'Drop #{drop} with marble #{marble}, from level {level}... marble breaks!')
       marble += 1
       high = level
       m -= 1
    else:
       print(f'Drop #{drop} with marble #{marble}, from level {level}... marble does not break!')
       low = level
if high == n + 1:
