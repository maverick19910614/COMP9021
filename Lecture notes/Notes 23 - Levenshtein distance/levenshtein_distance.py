...
class Levenshtein_distance:
    ...
    def __init__(self, word_1, word_2, insertion_cost = 1, deletion_cost = 1, substitution_cost = 2
        ...
        self._backtraces = [[self._table[i][j][1] for j in range(len(self.word_2) + 1)
    ...
    def _get_distances_and_backtraces_table(self):
        ...
        # each column being read from bottom to top, with word_1 and word_2
        ...
        # cost of converting the corresponding initial segment of word_1
        ...
        for i in range(1, N_1):
            for j in range(1, N_2):
                ...
                d['/'] = table[i - 1][j - 1][0] if self.word_1[i - 1] == self.word_2[j - 1]\
    ...
    def _compute_alignments(self, i, j):
        ...
        if '-' in self._backtraces[i][j]:
            yield from ((pair[0] + self.word_1[i - 1], pair[1] + '_')
                                                      for pair in self._compute_alignments(i - 1, j)
                       )
        if '|' in self._backtraces[i][j]:
            yield from ((pair[0] + '_', pair[1] + self.word_2[j - 1])
    ...
    def get_aligned_pairs(self):
        ...
        3653
        '''
        return list(self._compute_alignments(len(self.word_1), len(self.word_2)))
