...
class KMeansClustering(tk.Tk):
    ...
    def principle_help(self):
        tkinter.messagebox.showinfo('Principle',
            ...
            'After the user has created a number of (round) points, the button displaying '
            '"Cluster" can be clicked, and then the user can create k (square) points, or '
            '"centroids", displayed in different colors.\n'
            ...
            'The user can also click the button displaying "Stop" to get back to that position, '
            'and change her mind by clicking again on the button displaying "Cluster".'
                                    )

    def clearing_help(self):
        tkinter.messagebox.showinfo('Clearing',
            'In case centroids are displayed, clicking the "Clear" button deletes the centroids, '
            'and if the points are coloured because they have been clustered, then they lose their '
            'colour.\n\n'
    ...
    def creating_points_and_initial_centroids_help(self):
        tkinter.messagebox.showinfo('Creating points and initial centroids',
            ...
            'No point or centroid is created when it is too close to an existing point or '
...
class Space(tk.Frame):
    ...
    def __init__(self):
        ...
        self.space = tk.Canvas(self, width = self.space_dim, height = self.space_dim,
                                                                              bg = self.space_colour
                               )
    ...
    def clear(self):
        if self.centroids:
            ...
            for point_coordinates in self.points:
                ...
                self.space.itemconfig(self.points[point_coordinates].drawn_point,
        ...
        if self.clustering:
            ...
            if self.request_point_otherwise_delete_or_ignore(coordinates, self.points, 25):
                self.points[coordinates] = Point(self.space.create_oval(x - 2, y - 2, x + 2, y + 2,
                           fill = self.point_colour, outline = self.point_colour), self.point_colour
                                                )
    ...
    def draw_centroid(self, x, y, colour):
        return self.space.create_rectangle(x - 1, y - 1, x + 1, y + 1, fill = colour,
                                                                                    outline = colour
                                          )
